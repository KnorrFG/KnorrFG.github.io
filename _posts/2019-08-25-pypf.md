...
---
I love to play Pathfinder. And I've tried many different ways to manage my
...
you get a problem if you want to use custom items or house rules. This is
...
markdown and Excel, that would actually be very nice."
<!--more-->
...
a library out of it. In the following I will showcase this library but before we
...
on PyPi. To install it you will have to clone it, and then install it.
...
useful changes, please be so kind to send a pull-request. Since pip does not
support editable installations of of a *pyproject.toml* file, we will use [flit]
for that. Additionally we will need [Datasheet] too, so the complete setup
...
will = (4
        ...
        + abilities.Wis.Mod)
```
It is very straight forward, just gather all your information, and you can
...
It will take the base and bonus value for each ability and return a [AttrDict]
that represents this table with the columns *base*, *bonus*, *final* and
*modifier* for every ability. You can see in nearly every line below that
function call how you can use the table.

Then I add the stats per day table:
```python
...
The next thing are the skills. This can be really annoying: checking every
possible skill, checking the required ability, checking whether it can be used
untrained, noting whether it is a class skill so you can adapt it if you put
your first rank in at a later point in time ...
...
```
All you have to do is to enter the skills you have actually put points in.
...
fishy, better check against the list that is stored in *skills.py*.
...
```
All you have to do here is to add an entry for your class. Now there are only 3
things missing, that I want on my character sheet: the spells, the feats and the
items. Those are just ordinary dictionaries, and I will only show an example
here instead of the complete thing which you can find in
[the script](/assets/code/levi.py). 
...
spells_by_level = [
    ...
    ...]
```
Here you can actually put in the rules for the scaling and don't need to adjust
anything if you update your stats.
...
table I would need the following code:
...
The standalone was introduced just for this purpose and means that only a single
html file should be generated as output and nothing else.
All single line strings that contain a heading will automatically end up in the
TOC. All functions in the `pf.md` module will create markdown or html code
(which is valid markdown too). There are three relevant functions:

* `table()` for normal tables,
* `flat_table()` for tables with a single row 
* and `long_table()` for tables with a single column.
